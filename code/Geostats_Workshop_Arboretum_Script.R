...
# proj4string(ArboSP) <- CRS("+proj=utm +zone=18 ellps=WGS84")
...
# Plot on Google Satellite map
install.packages('RgoogleMaps')
library(RgoogleMaps)
ArboSP.ll <- spTransform(ArboSP, CRS("+proj=longlat +datum=WGS84")) # Transform coordinates to lat/long
mymap<-GetMap(center=c(mean(ArboSP.ll@coords[,2]),mean(ArboSP.ll@coords[,1])), zoom=15, maptype="hybrid")
PlotOnStaticMap(mymap,lat=ArboSP.ll@coords[,2],lon=ArboSP.ll@coords[,1],cex=1,pch=20,lwd=3,col=c('yellow'));
